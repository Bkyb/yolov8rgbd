...
from ultralytics.utils.metrics import OKS_SIGMA, PoseMetrics, kpt_iou, NEW_SIGMA
...
class PoseValidator(DetectionValidator):
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        ...
        self.sigma = OKS_SIGMA if is_pose else np.full(nkpt, NEW_SIGMA) # np.ones(nkpt) / nkpt
        # if is_pose:
        #     self.sigma = OKS_SIGMA
        # else:
        #     sigmas_np = np.full(nkpt, NEW_SIGMA * 1.5)
        #     special_indices = sorted(list(set([0, (nkpt - 1) // 2, nkpt - 1])))
        #     if special_indices:
        #         sigmas_np[special_indices] = NEW_SIGMA                    
        #     self.sigma = sigmas_np
